...
系统 SHALL 为「舆情速览」模块的每一个导航条目(舆情总览及全部子条目)提供一个真实可达的路由。点击任一条目 SHALL 导航到对应页面而非产生 404,且页面 SHALL 在控制台 shell(`ConsoleShell`)内渲染。**已交付真实能力的条目 SHALL 呈现真实内容**(舆情总览呈现「控制台分区」形态的数据看板:顶部 KPI rail + 中部三个 band + 右侧 sticky 信息流);尚未交付真实能力的条目 SHALL 展示该页面标题、说明与「功能建设中」空状态。条目的路由与导航 slug SHALL 在真实能力交付前后保持不变——交付仅替换页面正文,导航结构不变。
